        ...
        {showGraph ? (
              ...
              <CustomTable headerTitles={headerTitles}>
                {rewardStreams?.lockedStreams?.map(stream => (
                  <TableRow key={stream.start} buttonTitle="View">
                    ...
                    <TableCell>
                      <p>{(stream.amount?.[2] ?? stream.amount?.[3])?.toFixed(2)} mRLN</p>
                    </TableCell>
                  </TableRow>
                ))}
              </CustomTable>
          ...
          <EmptyState>
            ...
            {selectedSaveVersion === 1 ? (
              <p>Migrate your balance and deposit to the Vault to earn mRLN rewards.</p>
            ) : (
              <p>Deposit to the Vault to earn mRLN rewards.</p>
            )}
          </EmptyState>
        )}
      </div>
